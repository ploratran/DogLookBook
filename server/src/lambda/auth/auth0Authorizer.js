"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.handler = void 0;
require("source-map-support");
var jsonwebtoken_1 = require("jsonwebtoken");
var logger_1 = require("../../utils/logger"); // winston logger
var axios_1 = require("axios");
var logger = logger_1.createLogger('auth');
// You can read more about how to do this here: https://auth0.com/blog/navigating-rs256-and-jwks/
// Go to an Auth0 page -> Show Advanced Settings -> Endpoints -> JSON Web Key Set
var jwksUrl = 'https://dev-a6-tsbmc.us.auth0.com/.well-known/jwks.json'; // JWKS endpoint
exports.handler = function (event) { return __awaiter(void 0, void 0, void 0, function () {
    var jwtToken, e_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                logger.info('Authorizing a user', event.authorizationToken);
                _a.label = 1;
            case 1:
                _a.trys.push([1, 3, , 4]);
                return [4 /*yield*/, verifyToken(event.authorizationToken)];
            case 2:
                jwtToken = _a.sent();
                logger.info('User was authorized', jwtToken);
                return [2 /*return*/, {
                        principalId: jwtToken.sub,
                        policyDocument: {
                            Version: '2012-10-17',
                            Statement: [
                                {
                                    Action: 'execute-api:Invoke',
                                    Effect: 'Allow',
                                    Resource: '*'
                                }
                            ]
                        }
                    }];
            case 3:
                e_1 = _a.sent();
                logger.error('User not authorized', { error: e_1.message });
                return [2 /*return*/, {
                        principalId: 'user',
                        policyDocument: {
                            Version: '2012-10-17',
                            Statement: [
                                {
                                    Action: 'execute-api:Invoke',
                                    Effect: 'Deny',
                                    Resource: '*'
                                }
                            ]
                        }
                    }];
            case 4: return [2 /*return*/];
        }
    });
}); };
// verify token with event.authorizationToken:
function verifyToken(authHeader) {
    return __awaiter(this, void 0, void 0, function () {
        var token, jwt, header, key;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    token = getToken(authHeader) // get JWT token from event.authorizationToken
                    ;
                    jwt = jsonwebtoken_1.decode(token, { complete: true });
                    if (!jwt) {
                        logger.error('Invalid token');
                        throw new Error('401 error invalid token');
                    }
                    header = jwt.header;
                    return [4 /*yield*/, getSigningKey(jwksUrl, header.kid)
                        // verify with token and certificate
                    ];
                case 1:
                    key = _a.sent();
                    // verify with token and certificate
                    return [2 /*return*/, jsonwebtoken_1.verify(token, key.publicKey, { algorithms: ['RS256'] })];
            }
        });
    });
}
var getToken = function (authHeader) {
    if (!authHeader)
        throw new Error('No authentication header');
    if (!authHeader.toLowerCase().startsWith('bearer '))
        throw new Error('Invalid authentication header');
    var split = authHeader.split(' ');
    var token = split[1]; // get token from Auth Header passed into event
    return token;
};
// 3. Decode the JWT and grab the kid property from the header.
var getSigningKey = function (jwkurl, kid) { return __awaiter(void 0, void 0, void 0, function () {
    var res, keys, signingKeys, signingKey;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, axios_1["default"].get(jwkurl, {
                    // set Headers to allow CORS: 
                    headers: {
                        'Content-Type': 'application/json',
                        "Access-Control-Allow-Origin": "*",
                        'Access-Control-Allow-Credentials': true
                    }
                })];
            case 1:
                res = _a.sent();
                keys = res.data.keys;
                signingKeys = keys
                    .filter(function (key) {
                    return key.alg === 'RS256' // has algorithm as RS256s
                        && key.kty === 'RSA' // supporting RSA key type
                        && key.use === 'sig' // JWK property `use` determines the JWK is for signing
                        && key.kid // has 'kid' for unique key id
                        // has x5c property for public certificate
                        && ((key.x5c && key.x5c.length));
                })
                    .map(function (key) {
                    // return key with properties: kid, nbf and publicKey
                    return {
                        kid: key.kid,
                        nbf: key.nbf,
                        publicKey: certToPEM(key.x5c[0]) // certificate
                    };
                });
                if (!signingKeys.length) {
                    logger.error("No signing keys found");
                    throw new Error('Invalid signing keys');
                }
                signingKey = signingKeys.find(function (key) { return key.kid === kid; });
                if (!signingKey || !signingKeys.length) {
                    logger.error("No signing keys found");
                    throw new Error('Invalid signing keys');
                }
                logger.info("Signing keys created successfully ", signingKey);
                return [2 /*return*/, signingKey];
        }
    });
}); };
// 5. Using the x5c property build a certificate which will be used to verify the JWT signature. 
var certToPEM = function (cert) {
    // cert = cert.match(/.{1,64}/g).join('\n');
    cert = "-----BEGIN CERTIFICATE-----\n" + cert + "\n-----END CERTIFICATE-----\n";
    return cert;
};
//# sourceMappingURL=auth0Authorizer.js.map